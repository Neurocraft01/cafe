...
export default function ReservationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const whatsappNumber = "918262022502"; // +91 82620 22502
      const message = `*New Table Reservation*%0A%0A*Name:* ${formData.name}%0A*Phone:* ${formData.phone}%0A*Email:* ${formData.email}%0A*Date:* ${formData.date}%0A*Time:* ${formData.time}%0A*Guests:* ${formData.guests}%0A*Special Requests:* ${formData.specialRequests || 'None'}`;
      const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${message}`;
      
      // Open WhatsApp in new tab
      window.open(whatsappUrl, '_blank');

      // 2. Send data to our API (Resend + Google Calendar)
